...
def sugestao(request):
    ...
    if tipo == "C":
        ...
        empresas = Empresas.objects.filter(tempo_existencia__in=['+6', '+1']).filter(estagio__in=['MVP', 'MVPP'])
    elif tipo == 'D':
    ...
    for empresa in empresas:
        ...
        if percentual >= 1:
            empresas_selecionas.append(empresa)

    return render(request, 'sugestao.html', {'empresas': empresas_selecionas, 'areas': areas})
